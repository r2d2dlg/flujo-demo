...
def get_financial_costs_cashflow(db: Session = Depends(get_db)):
    ...
    try:
        ...
        for linea in lineas:
            ...
            # Track running balance for interest calculation
            running_balance = max(0, float(linea.monto_total_linea) - float(linea.monto_disponible))
            ...
            for month_idx, month_date in enumerate(months):
                ...
                # Update running balance before interest calculation
                running_balance += monthly_drawdowns
                
                # Calculate interest for the month on the running balance
                if linea.interest_rate:
                ...
                # Update running balance after interest and payments
                running_balance -= monthly_payments
