...
export default function ComisionesVentasTablePage() {
  const [data, setData] = useState<ComisionesVentasRow[]>([]);
  const [editingRow, setEditingRow] = useState<ComisionesVentasRow | null>(null);
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await api.get('/api/comisiones-ventas');
  ...
  const handleSave = async (formData: Partial<ComisionesVentasRow>) => {
    try {
      if (editingRow?.id) {
        await api.put(`/api/comisiones-ventas/${editingRow.id}`, formData);
        toast({
          title: 'Éxito',
        ...
        await api.post('/api/comisiones-ventas', formData);
        toast({
          title: 'Éxito',
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('¿Está seguro que desea eliminar este registro?')) {
      try {
        await api.delete(`/api/comisiones-ventas/${id}`);
        toast({
          title: 'Éxito',
  ...
  const handleBulkFillSubmit = async (startMonth: number, value: number, months: number) => {
    ...
    const updates: Partial<ComisionesVentasRow> = {};
    for (let i = 0; i < months && (startMonth + i) < 12; i++) {
      updates[`amount_${periods[startMonth + i].months[i]}`] = value;
    }

    try {
      await api.put(`/api/comisiones-ventas/${editingRow.id}`, updates);
      toast({
        title: 'Éxito',
  ...
  return (
    <Box p={4}>
      <HStack justify="space-between" mb={6}>
        <Heading size="lg">Comisiones de Ventas</Heading>
        <Button
          leftIcon={<AddIcon />}
          colorScheme="blue"
          onClick={() => {
      ...
      <Modal isOpen={isEditOpen} onClose={onEditClose} size="xl">
        ...
        <ModalContent>
          <ModalHeader>
            {editingRow?.id ? 'Editar' : 'Agregar'} Comisión
          </ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <VStack spacing={4}>
              <FormControl isRequired>
                ...
                <Input
                  ...
                  onChange={(e) => setEditingRow(prev => prev ? {...prev, concepto: e.target.value} : {
                    id: 0,
