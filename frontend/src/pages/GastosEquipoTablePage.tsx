...
export default function GastosEquipoTablePage() {
  const [data, setData] = useState<GastosEquipoRow[]>([]);
  const [editingRow, setEditingRow] = useState<GastosEquipoRow | null>(null);
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await api.get('/api/gastos_equipo');
  ...
  const handleSave = async (formData: Partial<GastosEquipoRow>) => {
    try {
      if (editingRow?.id) {
        await api.put(`/api/gastos_equipo/${editingRow.id}`, formData);
        toast({
          title: 'Éxito',
        ...
        await api.post('/api/gastos_equipo', formData);
        toast({
          title: 'Éxito',
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('¿Está seguro que desea eliminar este registro?')) {
      try {
        await api.delete(`/api/gastos_equipo/${id}`);
        toast({
          title: 'Éxito',
  ...
  const handleBulkFillSubmit = async (startMonth: number, value: number, months: number) => {
    ...
    const updates: Partial<GastosEquipoRow> = {};
    for (let i = 0; i < months && (startMonth + i) < 12; i++) {
      updates[`amount_${periods[startMonth + i].months[i]}`] = value;
    }

    try {
      await api.put(`/api/gastos_equipo/${editingRow.id}`, updates);
      toast({
        title: 'Éxito',
  ...
  return (
    <Box p={4}>
      <HStack justify="space-between" mb={6}>
        <Heading size="lg">Gastos de Equipo</Heading>
        <Button
          leftIcon={<AddIcon />}
          colorScheme="blue"
          onClick={() => {
