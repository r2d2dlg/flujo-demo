...
# --- CONFIGURATION ---
SOURCE_DIR = r"F:\projects\Grupo_11_Flujo"
DEST_DIR = r"F:\projects\flujo_demo"
NEW_DB_URL = "postgresql://arturodlg:Oc%d}c8lIm:9c2\\S@34.174.189.231:5432/demoflujo"

# Helper for robust deletion (handles Windows permission errors)
...
for root, dirs, files in os.walk(DEST_DIR):
    ...
    if 'backups' in dirs:
        shutil.rmtree(os.path.join(root, 'backups'), onerror=remove_readonly)
        print("Removed backups folder")
    if 'tableexport' in dirs:
        shutil.rmtree(os.path.join(root, 'tableexport'), onerror=remove_readonly)
        print("Removed tableexport folder")
    if 'Excel' in dirs:
        shutil.rmtree(os.path.join(root, 'Excel'), onerror=remove_readonly)
