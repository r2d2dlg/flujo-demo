...
export default function PagosTierraViewPage() {
  const [data, setData] = useState<PagosTierraViewRow[]>([]);
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await api.get('/api/pagos_tierra/view');
  ...
  return (
    <Box p={4}>
      <HStack justify="space-between" mb={6}>
        <Heading size="lg">Vista de Pagos a Tierra</Heading>
        <Button
          leftIcon={<RepeatIcon />}
          onClick={fetchData}
          colorScheme="blue"
