...
import { VPlanillaFijaConstruccion } from '../../../types/payrollTypes';

const PlanillaFijaConstruccionViewPage: React.FC = () => {
  const [data, setData] = useState<VPlanillaFijaConstruccion[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    payrollApi.getViewPlanillaFijaConstruccion()
      .then(res => setData(res.data))
      .catch(() => setError('No se pudo cargar la vista de Planilla Fija Construcción.'))
  ...
  return (
    <Box p={5}>
      <Heading mb={6}>Vista Planilla Fija Construcción</Heading>
      {loading ? (
...
export default PlanillaFijaConstruccionViewPage;
